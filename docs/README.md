...
- **Why it's credible.** The alert is not a dumb proximity buzzer. A per-pair risk
  ...
  actually _closing in_ — and only speaks when it has something worth saying.
...
| -------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------- |
| [`ARCHITECTURE.md`](./ARCHITECTURE.md) | System components, data flow, tech choices & rationale, event contracts, frontend design    | Explain _how it works_ to a technical judge |
